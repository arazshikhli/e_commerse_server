...
const getWishListProducts = async (req, res) => {
  ...
  try {
    ...
    const productIds = user.wishList.map(
      (wishListItem) => wishListItem.productId
    );
    const tvProducts = await TVSchema.find({ _id: { $in: productIds } });
    const mobileProducts = await MobileSchema.find({
      _id: { $in: productIds },
    });
    const desktopProducts = await LaptopSchema.find({
      _id: { $in: productIds },
    });
    const products = [...tvProducts, ...mobileProducts, ...desktopProducts];

    const fullWishList = user.wishList
      .map((wishListItem) => {
